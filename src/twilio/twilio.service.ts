...
import {
  ...
  TwilioCallInitiate,
  TwilioCallStatus,
  TwilioVoiceMailRedirections,
} from './dto/twilio.dto/twilio.dto';
...
export class TwilioService {
  ...
  handleCall(callInfo: TwilioCallInitiate): any {
    ...
    gather.say(
      'Thanks you for calling Turing Technologies!. Please press 1 to forward your call or press 2 to record a voicemail',
    );
  ...
  handleActions(data: TwilioCallActions): any {
    ...
    if (digit === '1') {
      ...
      response.say(
        'Please leave your voice message after the beep. Note you have a time limit of maximum 60 seconds. To Stop Recording Press Asterisk Key',
      );
      response.record({
        playBeep: true,
      ...
      response.say(
        'You have Entered an Incorrect Option. Thanks for Calling Turing Technologies!',
      );
      response.hangup();
    }
    return response.toString();
